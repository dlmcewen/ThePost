    ...
    <div class="container-fluid">
      <div class="row">
        ...
        <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
          ...
          <c:choose>
          	<c:when test="${updateItem == null }"> 
         		 <h2 class="sub-header">Select An Item To Update</h2>
         	</c:when>
         	<c:when test="${updateItem != null }">
         		<h2 class="sub-header">Update your item: ${updateItem.title}</h2>
         	</c:when>
         </c:choose>
          ...
          <h5>Select Item</h5>
          <form class="custom-form" role="form" action="SingleItemViewServlet" method ="POST">
            <select class="form-control" name="itemId">
            <option value = "return">--Select Item--</option>
            <c:forEach items="${currentSessionUser.itemList}" var="item"> 
       			<%-- Display an Item's title --%>
                <option value="${item.itemId}">${item.title}</option>
            </c:forEach>
          </select>
          <input type ="submit" value="Get Item Info"/>
          </form>
          ...
          <c:choose>
			<c:when test="${updateItem != null}">
		          ...
		          <form name="update" class="custom-form" role="form" action="UpdateItemServlet" method="POST" enctype="multipart/form-data" onsubmit="return validateForm()">
		          
		          <h5>Change Category</h5>
		          <select class="form-control" name="type" id="type">
		            <option value="${updateItem.type }">Current Type (${updateItem.type })</option>
		          ...
		          <h5>Change Title</h5>
		          <input type="text" value="${updateItem.title}" class="form-control" name="title" id="title"/>
		
		          <h5>Change Amount</h5>
		          <input type="text" value="${updateItem.price}" class="form-control" name="price" id="price"/>
		
		          <h5>Change Description</h5>
		          <textarea rows="4" class="form-control" name="description" id="description">${updateItem.description}</textarea>
				  <!-- 
				  <br>
		          ...
		          <input type="hidden" name="itemId" value="${updateItem.itemId}"/>
