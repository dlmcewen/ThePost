    ...
    <div class="container-fluid">
      <div class="row">
        ...
        <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
          ...
          <c:choose>
          	<c:when test="${adminRemoveItem == null }"> 
         		 <h2 class="sub-header">Select An Item To Remove</h2>
         	</c:when>
         	<c:when test="${adminRemoveItem != null }">
         		<h2 class="sub-header">Remove the item: ${adminRemoveItem.title}</h2>
         	</c:when>
         </c:choose>
          ...
          <c:choose>
			<c:when test="${adminRemoveItem != null}">
		          ...
		          <form name="remove" class="custom-form" role="form" action="AdminRemoveItemServlet" method="POST">
		          
		          <h2>Category:</h2>
		          <p>
		          <c:out value="${adminRemoveItem.type }"/>
				  </p>
		          <h2>Title:</h2>
		          <p>
		          <c:out value="${adminRemoveItem.title }"/>
				  </p>
		          <h2>Amount:</h2>
		          <p>
		          <c:out value="${adminRemoveItem.formatedPrice }"/>
				  </p>
		          <h2>Description:</h2>
		          <p>
		          <c:out value="${adminRemoveItem.description }"/>
				  </p>
				  <h2>Image:</h2>
				  <img src="${adminRemoveItem.imageLocation}" width="200px" height="200px"><br><br>
				  
				  
		          <input type="hidden" name="itemId" value="${adminRemoveItem.itemId}"/>
		
		          <button type="submit" value="Submit" class="btn btn-primary">Remove Listing</button>
		       </form>
		     </c:when>
		    </c:choose>
